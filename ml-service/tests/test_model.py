...
class TestSinglePrediction:
    ...
    def test_predict_returns_tuple(self, predictor):
        res = predictor.predict(2, 1, 3, 50, 1.0)
        time, conf = res[0], res[1]
        assert isinstance(time, float)
        assert isinstance(conf, float)

    def test_predict_positive_time(self, predictor):
        """Predictions must always be positive."""
        time = predictor.predict(1, 1, 1, 0, 0.5)[0]
        assert time > 0
    ...
    def test_predict_confidence_range(self, predictor):
        ...
        for size in [1, 2, 3]:
            for load in [0, 50, 100]:
                conf = predictor.predict(size, 1, 3, load, 1.0)[1]
    ...
    def test_monotonicity_size(self, predictor):
        ...
        time_small = predictor.predict(1, 1, 3, 50, 1.0)[0]
        time_large = predictor.predict(3, 1, 3, 50, 1.0)[0]
    ...
    def test_feasibility_guard(self, predictor):
        ...
        time = predictor.predict(1, 1, 1, 0, 2.0)[0]
    ...
    def test_clipping_extreme_inputs(self, predictor):
        ...
        res = predictor.predict(99, 99, 99, 999, 100)
        time, conf = res[0], res[1]
        assert isinstance(time, float)
        assert time > 0
    ...
    def test_no_nan_outputs(self, predictor):
        ...
        for size in [1, 2, 3]:
            for ttype in [1, 2, 3]:
                for pri in [1, 3, 5]:
                    res = predictor.predict(size, ttype, pri, 50, 1.0)
                    time, conf = res[0], res[1]
    ...
    def test_deterministic(self, predictor):
        ...
        res1 = predictor.predict(2, 2, 3, 50, 1.0)
...
class TestBatchPrediction:
    ...
    def test_batch_consistency_with_single(self, predictor):
        ...
        res_single = predictor.predict(*features)
        single_time, single_conf = res_single[0], res_single[1]
        batch_results = predictor.predict_batch([features])
        batch_time, batch_conf = batch_results[0][0], batch_results[0][1]
